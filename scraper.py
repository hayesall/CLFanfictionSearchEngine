...
class Scraper:
    ...
    def scrape_story_metadata(self, story_id):
        ...
        # if there are two, the second is the time it was updated last
        # -Alexander
        if len(times) == 1:
            when_updated = int(times[0]['data-xutime'])
            when_published = int(times[0]['data-xutime'])
        elif len(times) == 2:
            when_updated = int(times[0]['data-xutime'])
            when_published = int(times[1]['data-xutime'])
        ...
        metadata = {
            ...
            # swapping these out to deal with oneshots, they must not be as common as they once were.
    ...
    def scrape_story(self, story_id, keep_html=False):
        ...
        print 'Found story: "%s", with id: "%s"' % (str(metadata['title']), str(story_id))

        #It's possible for num_chapters to be 1, meaning that further
        #scraping is unecessary. --Alexander
        num_chapters = metadata.get('num_chapters')
        if not num_chapters:
            #num_chapters = 1
